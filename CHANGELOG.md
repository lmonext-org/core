...
- Changelog: addon.json 1.8.1 / view_addons.php 2.5.0 - Stilkorrektur (auf Wunsch): sämtliche PHP-Kurzsyntax-Kontrollstrukturen (if/elseif/else: ... endif;, foreach: ... endforeach;) auf die im übrigen Code durchgängig verwendete geschweifte-Klammer-Syntax umgestellt - keine Verhaltensänderung, rein mechanisch. War die einzige Datei im gesamten Projekt (neben einer einzelnen Stelle in admin/view_liga_settings.php, siehe dortiger Changelog), die noch die alternative Syntax nutzte - vermutlich ein Rest aus der allerersten addon-manager-Version, an dem sich der Stil seither nicht mehr geändert hatte. Zwei verschachtelte Mehrzeilen-Fälle (Update-Badge-Rendering, Auto-Purge-Button) wurden dabei von Hand nachkorrigiert, da eine rein zeilenbasierte automatisierte Umwandlung sie zunächst falsch verschachtelt hätte.

- Changelog: addon.json 1.8.0 / view_addons.php 2.4.0 / catalog.php 1.0.0 (neu) - Neuer Tab "🧭 Entdecken" (auf Wunsch): zeigt alle offiziellen LMOnext-Addons aus dem neuen, statischen Katalog catalog.php (Name, Icon, Kurzbeschreibung, Autor, GitHub-Link - Datenquelle: dieselbe Addon-Datenbank wie die Projekt-Website, hier auf die für die Direktinstallation nötigen Felder reduziert) mit direktem "Installieren"-Button pro Addon. Der Button nutzt exakt den bereits vorhandenen Weg "install_from_url" (AddonManager::installFromGithubUrl(), siehe dessen Changelog) mit vorausgefüllter repo_url aus dem Katalog - keine neue Installationslogik, keine neuen Sicherheitsprüfungen nötig. Bereits installierte Katalog-Addons werden stattdessen mit ihrem Aktiv/Inaktiv-Status markiert (kein Install-Button mehr); ihr weiteres Update läuft unverändert über den bestehenden GitHub-Update-Mechanismus (checkGithubUpdates()/installUpdate(), liest die Repo-URL aus der eigenen addon.json des Addons, unabhängig vom Installationsweg). Respektiert den Demo-Modus wie die anderen Installationswege. Auf einer frischen Installation (nur das stets vorhandene Core-Addon "addon-manager" selbst, sonst nichts) ist "Entdecken" jetzt der Standard-Tab statt der leeren "Alle"-Liste; der "Alle"-Tab weist im leeren Zustand zusätzlich auf "Entdecken" hin. Enthält außerdem einen Hinweis, dass inoffizielle Addons weiterhin ganz normal über "⚙ Einstellungen" (ZIP-Upload oder beliebige GitHub-URL) installierbar sind - diese beiden Wege gab es technisch bereits unverändert seit view_addons.php 2.3.0, waren aber nirgends explizit als "auch für inoffizielle Addons geeignet" beworben.

- Changelog: handler_addons.php 1.5.0 - KRITISCHER Bugfix (gemeldet: "Core-Version zu niedrig. Benötigt: 1.9.2, vorhanden: 1.9.2-beta" bei einem Addon, das nachweislich kompatibel war): die Min-Core-Version-Prüfung beim Aktivieren eines Addons nutzte direkt version_compare(), das ein "-beta"-Suffix als "kleiner" als dieselbe Zahl ohne Suffix einstuft (technisch korrekt nach SemVer, hier aber falsch angewendet - der Suffix beschreibt die laufende Core-Version selbst, nicht eine andere, ältere Version). Nutzt jetzt AddonManager::versionSatisfiesMin() 1.10.0 (siehe dortiger Changelog-Eintrag für die vollständige Korrektur), die Pre-Release-Suffixe vor dem Vergleich entfernt.
...
- Changelog: 1.7.0 - Route umbenannt: "liga_detail" rendert keine View mehr (springt in admin/data_loader.php 1.15.0 direkt zum aktuellen Spieltag), die bisherige Spieltage-Übersicht läuft jetzt unter der neuen Aktion "liga_spieltage" (admin/view_liga_spieltage.php, vormals view_liga_detail.php).
...
- Changelog: 1.28.0 - Zwei neue Funktionen für den Liga-Direkteinstieg (auf Wunsch): resolveLigaEntrySpieltagNr() ermittelt beim Öffnen einer Liga den anzuzeigenden Spieltag (1. zuletzt angesehener Spieltag dieser Liga, sonst 2. erster Spieltag mit fehlenden Ergebnissen, sonst 3. letzter Spieltag) und rememberLigaLastSpieltag() merkt sich den zuletzt in der Ergebniseingabe angesehenen Spieltag je Liga (liga_options "LastSpieltagNr"). Keine Schema-Änderung nötig, liga_options ist bereits eine generische Key-Value-Tabelle.
- Changelog: 1.27.0 - Neue Spalte gt_grund (TEXT NULL) für liga_partien (auf Wunsch: freier Zusatztext zur Grüne-Tisch-Entscheidung), defensive Migration nach demselben Muster wie gt_entscheidung.
...
- Changelog: 1.25.0 - Neues Feature (auf Wunsch, nach DFB-Rechts- und Verfahrensordnung): ensureSpielstatusColumns() erweitert um die Spalte `gt_entscheidung` auf liga_partien (0=keine, 1=Heimteam siegt, 2=Gastteam siegt am grünen Tisch) - siehe StandingsTrait::gtCreditedScore() für die vollständige Wertungsregel.
- Changelog: 1.24.0 - Neues Feature (auf Wunsch): ensureSpielstatusColumns() erweitert um die additive Spalte `nicht_gewertet` auf liga_partien - erlaubt es, ein Spiel rückwirkend zu annullieren (zählt für keines der beiden beteiligten Teams in der Tabelle), z.B. bei Lizenzentzug/Spielbetrieb-Einstellung eines Vereins mitten in der Saison. Bewusst eine EIGENE, von "status" (i.E./n.V.) unabhängige Spalte statt eines dritten status-Werts, damit ein annulliertes Elfmeterschießen-Spiel diese Information nicht verliert, falls die Annullierung später wieder aufgehoben wird.
...
- Changelog: 1.10.0 - KRITISCHER Bugfix (gemeldet: nach Annullierung eines einzelnen Teams (Vereins-Spielbetrieb-Einstellung, z.B. Türkgücü München) zeigte die Strafen-Übersicht plötzlich ALLE Teams der Liga mit angehaktem "Annulliert" an, obwohl nur eines tatsächlich betroffen war - die Tabellenansicht selbst blieb davon unberührt und korrekt): die Abfrage prüfte bisher nur "hat dieses Team MINDESTENS EIN annulliertes Spiel" statt "sind ALLE Spiele dieses Teams annulliert". Da jedes andere Team der Liga mindestens einmal gegen das zurückgezogene Team spielt und genau dieses eine Spiel ja ebenfalls annulliert wird (eine Annullierung betrifft immer beide Seiten eines Spiels, siehe computeStandings()), erfüllte praktisch jedes Team der Liga die alte, zu schwache Bedingung. Auf GROUP BY + HAVING umgestellt (Summe der NICHT annullierten Spiele je Team muss 0 sein). Mit einer simulierten 3-Team-Datenbank verifiziert: alte Abfrage lieferte fälschlich alle 3 Teams, neue Abfrage korrekt nur das eine tatsächlich zurückgezogene Team. Da ein unbemerktes Speichern mit allen fälschlich angehakten Checkboxen die gesamte Liga annulliert hätte, war dies potenziell folgenreich, auch wenn im gemeldeten Fall noch nicht gespeichert wurde.
- Changelog: 1.9.0 - Liga-Einstellungen (Strafen-Tab) laden jetzt zusätzlich, welche Teams in dieser Liga bereits vollständig annulliert sind (liga_partien.nicht_gewertet), damit view_liga_settings.php die entsprechende Checkbox korrekt vorbelegen kann.
...
- Changelog: 1.14.0 - KRITISCHER Bugfix, gefunden beim Ergänzen von gt_grund: die Validierung "$gt > 2" an beiden Speicherstellen (auch der tatsächlich vom Formular verwendeten save_partie_teams) stammte noch von vor der Einführung der Grüne-Tisch-Option 3 ("beide Mannschaften verlieren", siehe StandingsTrait.php 1.12.0) und hätte diese IMMER still auf 0 zurückgesetzt - nie tatsächlich gespeichert. Jetzt auf "$gt > 3" korrigiert. Außerdem (auf Wunsch): speichert jetzt zusätzlich gt_grund an beiden Stellen.
- Changelog: 1.13.0 - KRITISCHER Bugfix (gemeldet: Grüne-Tisch-Entscheidung wurde nach dem Speichern immer wieder auf "–" zurückgesetzt, in der Besucheransicht erschien nichts): die Änderung in 1.11.0 hatte den falschen Handler erweitert - save_ergebnisse existiert zwar noch im Code, wird aber von KEINEM Formular mehr aufgerufen (toter Code). Das tatsächlich von view_spieltag.php genutzte Formular (reguläre Liga-Ansicht mit Team-Auswahl-Dropdowns) sendet an "save_partie_teams" - dessen UPDATE-Statement enthielt weder status, noch bericht_url, noch gt_entscheidung. Alle drei jetzt ergänzt (status/bericht_url fehlten bereits vor der Grüne-Tisch-Entscheidung und wurden bei der Gelegenheit gleich mit behoben - betraf vermutlich auch "nach Verlängerung"/"im Elfmeterschießen" und den Spielbericht-Link, nur bisher nicht gemeldet).
- Changelog: 1.12.0 - Neuer Hook-Punkt "liga.deleted" an der Stelle ergänzt, an der eine Liga tatsächlich gelöscht wird - gefunden bei der Integrationsprüfung eines Drittanbieter-Addons (liga-notizen), das sich bereits auf dieses Event verließ, obwohl es zuvor im Core gar nicht existierte (analog zu "team.deleted" in 1.10.0). Erlaubt Addons, eigene, an die Liga gebundene Daten zu bereinigen, statt verwaist in der Datenbank zurückzubleiben. Bewusst ERST NACH dem commit() der Transaktion gefeuert (dieser Löschvorgang läuft bereits in einer eigenen Transaktion, um verwaiste Zeilen in liga_options/liga_teams/liga_team_values/liga_spieltage/liga_partien zu vermeiden), damit ein Rollback nicht fälschlich schon Aufräumarbeiten eines Addon-Hooks auslöst.
- Changelog: 1.11.0 - FEHLERHAFT/WIRKUNGSLOS (siehe Korrektur in 1.13.0): save_ergebnisse verarbeitet jetzt zusätzlich das neue gt_<id>-Feld (Grüne-Tisch-Entscheidung, siehe admin/view_spieltag.php) und speichert es in liga_partien.gt_entscheidung.
- Changelog: 1.10.0 - Neuer Hook-Punkt "team.deleted" an allen drei Stellen ergänzt, an denen ein globales Team tatsächlich gelöscht wird (delete_global_team, Platzhalter-Team-Bereinigung beim "Team aus DB übernehmen", Duplikat-Löschung beim Zusammenführen zweier Teams) - gefunden bei der Integrationsprüfung eines Drittanbieter-Addons, das sich bereits auf dieses Event verließ, obwohl es zuvor im Core gar nicht existierte. Erlaubt Addons, eigene, an das Team gebundene Daten zu bereinigen, statt verwaist in der Datenbank zurückzubleiben. Beim Zusammenführen-Workflow bewusst ERST NACH dem commit() der Transaktion gefeuert, damit ein Rollback nicht fälschlich schon Aufräumarbeiten eines Addon-Hooks auslöst. Erste Nutzung von doHook() im Admin-Bereich (bisher nur im Frontend etabliert) - verifiziert, dass die AddonManager-Instanz in admin.php global registriert wird und doHook() dort ebenso funktioniert.
...
- Changelog: 1.12.0 - Speichert jetzt zusätzlich tickernotizen/tickerbreite/tickergeschwindigkeit sowie den erweiterten tickerart-Wert "ergebnisse_favorit" (auf Wunsch: Favoriten-Ergebnisticker, Spielnotizen, konfigurierbare Breite/Geschwindigkeit - siehe addon/ticker/TickerRenderer.php 1.1.0 im ticker-Addon für den fachlichen Hintergrund), an beiden Speicherstellen, weiterhin mit isEnabled('ticker')-Absicherung.
- Changelog: 1.11.0 - Ticker jetzt als eigenständiges Addon "ticker" ausgegliedert (auf Wunsch, Nutzer erinnerte daran, dass der Newsticker im alten LMO4 ebenfalls im addon/-Ordner lag, nicht im Core): beide Speicherstellen (Haupt-Tab und der eigenständige "ticker"-Speicher-Endpunkt) jetzt mit isEnabled('ticker') abgesichert, analog zum bereits bestehenden player-Addon-Muster - ohne aktives Addon wird nichts gespeichert, das Formularfeld ist ja auch ausgeblendet.
...
## admin/view_liga_spieltage.php

- Changelog: 1.0.0 - Umbenennung von admin/view_liga_detail.php (auf Wunsch, siehe admin/data_loader.php 1.15.0 fuer den Hintergrund): zeigt unveraendert die Spieltage-Uebersichtstabelle einer Liga (inkl. Startdatum-Bearbeitung), jetzt aber unter der Aktion "liga_spieltage" statt "liga_detail" - diese Aktion ist neu der intelligente "Liga oeffnen"-Direkteinstieg und rendert keine eigene View mehr. Der Navigationsblock (Liganame, Buttons, Archiv-Dropdown, KO-Rundenhinweis) wurde in das neue gemeinsame Partial admin/view_liga_nav.php ausgelagert und wird hier per require_once eingebunden; die aeltere Historie dieses Blocks steht weiterhin unter dem alten Abschnitt "admin/view_liga_detail.php" unten.

## admin/view_liga_nav.php

- Changelog: 1.0.0 - Neu (auf Wunsch): gemeinsamer Navigationsblock einer Liga (Name/Typ, ID/Erstellt, Aktions-Buttons, Archivieren-Dropdown, KO-Fehlende-Runden-Hinweis), extrahiert aus dem bisherigen admin/view_liga_detail.php (siehe dessen Historie), damit derselbe Block sowohl auf der Spieltage-Uebersicht (admin/view_liga_spieltage.php) als auch bei der Ergebniseingabe eines einzelnen Spieltags (admin/view_spieltag.php 1.9.0) angezeigt werden kann. Neuer Button "Spieltage-Uebersicht" verlinkt auf die Aktion "liga_spieltage". Enthaelt jetzt auch das JS fuer das Archiv-Dropdown (toggleArchivMenu()), vorher direkt in view_liga_detail.php.
...
- Changelog: 1.18.0 - Stilkorrektur (auf Wunsch, siehe addon/addon-manager/view_addons.php 2.5.0 für den vollständigen Hintergrund): die einzige Stelle in dieser Datei mit PHP-Kurzsyntax (if/else: ... endif;) auf geschweifte-Klammer-Syntax umgestellt. Keine Verhaltensänderung.
...
- Changelog: 1.16.0 - Neue Ticker-Einstellungen im Anzeigen/Darstellung-Tab: dritte Ticker-Art "Ergebnisticker (nur Lieblingsmannschaft)", Checkbox "Spielnotizen anzeigen?", sowie Breite (Zeichen) und Geschwindigkeit (Zeichen/Minute) - siehe admin/handler_settings.php 1.12.0.
- Changelog: 1.15.0 - Der komplette Ticker-Einstellungsbereich (Ein/Aus, Ticker-Art, freier Text) ist jetzt mit isEnabled('ticker') umschlossen, siehe admin/handler_settings.php 1.11.0 für den vollständigen Hintergrund zur Auslagerung als eigenständiges Addon.
...
- Changelog: 1.9.0 - Neues Feature im Strafen-Tab (auf Wunsch): neue Spalte "Alle Spiele annullieren" mit einer Checkbox pro Team - setzt beim Speichern ALLE Spiele dieses Teams in dieser Liga auf "nicht gewertet" (siehe admin/handler_settings.php). Bereits annullierte Teams werden farblich hervorgehoben angezeigt.
...
- Changelog: 1.9.0 - Auf Wunsch: gemeinsamer Liga-Navigationsblock (admin/view_liga_nav.php, neu) oben eingebunden - dieselben Aktions-Buttons wie auf der Spieltage-Übersicht sind jetzt auch bei der Ergebniseingabe sichtbar, inkl. neuem Link "Spieltage-Übersicht". Der dadurch überflüssige "← Liganame"-Rücksprunglink wurde entfernt. Die Schnellnavigation "Alle Spieltage"/"Alle Runden" wurde vom Seitenende direkt unter den Navigationsblock verschoben (und steht jetzt über statt unter dem Ergebniseingabe-Formular).
- Changelog: 1.8.0 - Feature (auf Wunsch): neues Freitextfeld für zusätzliche Informationen zur Grüne-Tisch-Entscheidung, als eigene Zeile unterhalb jeder Partie (analog zum bestehenden Spielbericht-Link-Feld) - per JavaScript nur eingeblendet, wenn eine Entscheidung im Sportgericht-Dropdown ausgewählt ist (initial serverseitig anhand des gespeicherten Werts, danach per change-Listener auf dem Dropdown).
- Changelog: 1.7.0 - Neue dritte Option "Beide Mannschaften verlieren" im Grüne-Tisch-Entscheidung-Dropdown im Ergebniseditor (Wert 3, neben den bestehenden 1/2 für Heim-/Gastsieg) - siehe src/Liga/StandingsTrait.php 1.12.0.
- Changelog: 1.6.0 - Korrektur (auf Wunsch, fachlich begründet): die Spalte "Status" (i.E./n.V.) in der regulären Liga-Ansicht wird jetzt nur noch angezeigt, wenn die Liga NICHT Fußball ist. In einer normalen Fußball-Ligabegegnung gibt es diese Beendigungsarten schlicht nicht (nur bei einem entscheidungspflichtigen KO-Spiel) - anders als bei der Grüne-Tisch-Entscheidung, die sportart- und ligaform-übergreifend gültig bleibt, war diese Spalte für eine Fußball-Liga funktional bedeutungslos und wurde komplett ausgeblendet statt nur inaktiv gelassen (nutzt die bereits vorhandene $ligaSportType-Abfrage, siehe Kommentar dort). Für andere Sportarten (z.B. Eishockey, wo Verlängerung/Penalty-Shootout auch in der regulären Saison vorkommen) bleibt die Spalte unverändert sichtbar - bewusst nicht pauschal für alle Sportarten entfernt. Spaltenanzahl der Notiz-/Sätze-Zeilen (colspan) entsprechend dynamisch angepasst ($tableColCount).
- Changelog: 1.5.0 - Neue Spalte "Sportgericht" in der Ergebniseingabe (auf Wunsch, nach dem Vorbild des alten LMO4): Dropdown pro Spiel für eine Grüne-Tisch-Entscheidung (–/Heimteam siegt/Gastteam siegt), rot hervorgehoben wenn aktiv. Nur in der regulären Liga-Ansicht ergänzt (nicht im KO-Modus-Baum, dort bisher nicht angefragt).
- Changelog: 1.4.1 - Sicherheitsfix: csrfField() in jedes POST-Formular eingefügt (CSRF-Schutz, siehe admin/bootstrap.php).
...
- Changelog: 1.11.1 - Core-Versionssprung für die Stilkorrektur PHP-Kurzsyntax → geschweifte Klammern (auf Wunsch, siehe addon/addon-manager/view_addons.php 2.5.0 und admin/view_liga_settings.php 1.18.0). Reine Code-Stilfrage, keine Verhaltensänderung.

- Changelog: 1.11.0 - Core-Versionssprung für den neuen "Entdecken"-Tab im Addon-Manager (auf Wunsch, siehe addon/addon-manager/ - addon.json 1.8.0/view_addons.php 2.4.0/catalog.php 1.0.0): offizieller Addon-Katalog mit Direktinstallation per Klick.

- Changelog: 1.10.2 - Core-Versionssprung für die drei install.php-Härtungen (auf Wunsch, siehe install.php 2.9.0): CSRF-Schutz fürs Installationsformular, explizite MySQL/MariaDB-Mindestversionsprüfung, Bestätigungs-Checkbox bei bereits vorhandener Installation unter demselben Host/DB-Name/Präfix.

- Changelog: 1.10.1 - Core-Versionssprung für die install.php-Überarbeitung (auf Wunsch, siehe install.php 2.8.0): vollständigeres DB-Schema von Anfang an (team_links, addon_registry, addon_settings + mehrere fehlende Spalten), Rechte-Vorabprüfung, abschließende Konsistenzprüfung, entfernter bin/composer.phar-Rückfall (bin/-Ordner komplett aus dem Core gelöscht) sowie mehrere Sicherheits-Härtungen (config.php-Dateiberechtigung, Schutz vor Neuinstallation über eine bestehende config.php).
- Changelog: 1.10.0 - Hochstufung aus der Beta-Phase: kein "-beta"-Suffix mehr. Rein administrative Versionsänderung (auf Wunsch, nach ausgiebigem beschwerdefreiem Betrieb) - keine Code-Änderung, da die Version überall (Footer, Admin-Sidebar, Backup-Dateien, Addon-Kompatibilitätsprüfung) einheitlich per getAppVersion()/renderCopyrightNotice() direkt aus composer.json gelesen wird. AddonManager::versionSatisfiesMin() (1.10.0, siehe dortiger Changelog) entfernt Pre-Release-Suffixe ohnehin schon vor dem Vergleich, ein Addon mit min_core_version "1.10.0-beta" bleibt also weiterhin kompatibel.
- Changelog: 1.10.0-beta - Core-Versionssprung für das neue "Liga öffnen springt direkt zum aktuellen Spieltag"-Feature (auf Wunsch, siehe admin/bootstrap.php 1.28.0, admin/data_loader.php 1.15.0, neues Partial admin/view_liga_nav.php 1.0.0, admin/view_spieltag.php 1.9.0, admin/view_liga_detail.php umbenannt zu admin/view_liga_spieltage.php 1.0.0).
- Changelog: 1.9.5-beta - Core-Versionssprung, der die seit 1.9.2-beta angesammelten Änderungen zu einer neuen, für Addons referenzierbaren min_core_version zusammenfasst: neue Core-Hook-Punkte "team.deleted" und "liga.deleted" (ermöglichen Addons, eigene an ein Team/eine Liga gebundene Daten beim Löschen automatisch zu bereinigen), struktureller Bugfix an der Addon-Boot-Reihenfolge (bootAdmin()/bootFrontend() laufen jetzt in drei sauber getrennten Phasen: Sprachen laden → Handler-Dateien laden → Hooks registrieren, behebt sowohl ein Übersetzungs-Cache- als auch ein Hook-Registrierungs-Problem), AVAILABLE_LANGUAGES-basierte Addon-Sprachladung statt einer hartkodierten de/en-Liste, sowie zwei neue, optionale Spalten auf liga_partien (nicht_gewertet für rückwirkend annullierte Spiele, gt_entscheidung für Grüne-Tisch-Entscheidungen nach DFB-Regel) mit entsprechender Berücksichtigung in der Tabellenberechnung.
...
- Changelog: ENTFERNT - diese Datei wurde vollständig gelöscht (auf Wunsch, nach gründlicher Prüfung). Sie war seit unbekannter Zeit vollständiges, totes Duplikat: kein require_once/include irgendwo im Core oder in einem der geprüften Addons (komplette Addon-Sammlung des Nutzers durchsucht) referenzierte diese Datei - nur wenige Kommentare in anderen Dateien erwähnten sie textuell, aus einer Zeit, bevor dies entdeckt wurde. Von den 61 Funktionen dieser Datei existierte nur formatScoreWithGt() nirgends sonst - diese wurde ausschließlich innerhalb der Datei selbst genutzt (kein externer Aufrufer, auch nicht in Addons), musste also nirgendwo integriert werden. Alle anderen 60 Funktionen hatten längst ein aktives Äquivalent in frontend/data_liga.php (dünne Wrapper zu src/Liga/*), das tatsächlich geladen wird. Frühere Changelog-Einträge dieser Datei (unten) bleiben als Historie erhalten.
- Changelog: 2.32.0 - getSpieltagPartien() (eigenständige, tote Duplikat-Implementierung) sortiert ebenfalls primär nach Anstoßzeit statt spiel_nr, analog zu src/Liga/SpieltagRepositoryTrait.php 1.6.0.
- Changelog: 2.31.0 - resolveTeamNumberToId() (eigenständiges, totes Duplikat) aus Konsistenzgründen ebenfalls von Positions-Auflösung auf reine Gültigkeits-Prüfung umgestellt, analog zu src/Liga/TeamRepositoryTrait.php 1.1.0.
- Changelog: 2.30.0 - Default-Fallback für _gt_tore_nichtantritt in formatScoreWithGt() (totes Duplikat) von 3 auf 2 geändert, konsistent mit src/Liga/RenderViewsTrait.php 1.21.0.
- Changelog: 2.29.0 - formatScoreWithGt() (eigenständiges, totes Duplikat, siehe frühere Einträge) aus Konsistenzgründen ebenfalls auf die pro Liga einstellbaren Grüne-Tisch-Standardwerte umgestellt, analog zu src/Liga/RenderViewsTrait.php 1.20.0.
...
- Changelog: 2.24.0 - getSpieltagPartien() und statusSuffix() (beides eigenständige Implementierungen in dieser Datei, keine Wrapper) um dieselbe nicht_gewertet-Unterstützung ergänzt wie in src/Liga/SpieltagRepositoryTrait.php bzw. TeamFormattingTrait.php - sonst hätte dieses alternative Template die neue Annullieren-Funktion nicht angezeigt.
...
- Changelog: 2.9.0 - Die drei zuletzt offen gelassenen Härtungs-Ideen umgesetzt (auf Wunsch): (1) CSRF-Schutz fürs Installationsformular - pro Session ein Zufalls-Token (INSTALL_CSRF_TOKEN, session-basiert), als verstecktes Feld in Schritt 2 mitgeschickt und beim POST per hash_equals() geprüft (err_csrf_invalid bei Mismatch). (2) Mindestversion von MySQL/MariaDB wird jetzt explizit geprüft (checkServerVersionSupportsJson(), direkt nach dem Verbindungsaufbau, vor jedem Tabellen-Zugriff) - mindestens MySQL 5.7.8 bzw. MariaDB 10.2.7 wegen des JSON-Spaltentyps auf liga_partien.extra_data (Sport-Profile-Erweiterung); erkennt dabei auch den "5.5.5-"-Compat-Versionspräfix mancher MariaDB-Server. (3) Neuer Hinweis samt Bestätigungs-Checkbox, falls unter dem gewählten Host/Datenbankname/Tabellen-Präfix bereits eine Installation mit Daten liegt (detectExistingInstallation() - prüft, ob {prefix}admin_users existiert und mindestens einen Eintrag hat). Blockiert NICHT hart (eine Installation über eine bestehende DB bleibt ein bewusst unterstütztes Feature, z.B. um den Admin-Zugang zurückzusetzen), verlangt aber eine ausdrückliche Bestätigung (Checkbox "confirm_existing"), damit niemand aus Versehen eine falsche/fremde Datenbank überschreibt, nur weil Host/Name/Präfix zufällig übereinstimmen.

- Changelog: 2.8.0 - Umfassende Überprüfung/Absicherung auf Wunsch: (1) Schema-Vollständigkeit nachgezogen - drei komplett fehlende Tabellen ergänzt, die bisher nur über Laufzeit-Migrationen in admin/bootstrap.php entstanden (`team_links`, `addon_registry`, `addon_settings` - Letztere zwei für den Addon-Manager, der zu den Kernfunktionen zählt), sowie fehlende Spalten (`teams_global.url`, `admin_users.last_login`, `liga_partien.nicht_gewertet`/`gt_entscheidung`/`gt_grund`) inkl. passender ALTER-Migrationen für Bestandsinstallationen. (2) Neue Rechte-Vorabprüfung: legt vor den eigentlichen ~16 Tabellen eine Wegwerf-Tabelle an/ändert/löscht sie, um fehlende CREATE/ALTER/DROP-Rechte sofort mit klarer Meldung statt mitten im Ablauf zu erkennen. (3) Neue abschließende Konsistenzprüfung nach dem Schreiben: liest jede Tabelle per SHOW COLUMNS zurück und vergleicht sie mit den aus den CREATE-TABLE-Strings selbst extrahierten Erwartungen (verifyDatabaseConsistency(), extractTableNameFromSql(), extractExpectedColumns() - keine zweite, separat zu pflegende Spaltenliste) sowie ob der Administrator-Account wirklich angelegt wurde; bei Inkonsistenzen wird KEINE config.php geschrieben, sondern ein klarer Fehler angezeigt. Dabei nebenbei einen vorbestehenden Formatierungsfehler in der liga_archiv_folders-Definition behoben (fehlendes Leerzeichen nach `beschreibung`, hätte die neue Prüfung sonst fälschlich als fehlende Spalte gemeldet). (4) Composer-Rückfall auf eine mitgelieferte bin/composer.phar entfernt (auf Wunsch) - installComposerDependencies() versucht jetzt ausschließlich ein systemweit installiertes composer-Kommando; schlägt das fehl oder ist keins vorhanden, wird direkt (ohne PHP-CLI-Suche, ohne .phar) auf die klassische config.php zurückgefallen. findPhpCli() dadurch komplett entfernt, COMPOSER_PHAR-Konstante entfernt, der komplette bin/-Ordner (composer.phar + dessen .htaccess) aus dem Core gelöscht. composerStatus() (vorher totes, nie aufgerufenes Feld) jetzt als sichtbarer, optionaler Check in Schritt 1 verwendet, damit transparent ist, welcher der beiden gleichwertigen Wege genutzt wird. (5) Weitere Härtung: config.php wird nach dem Schreiben jetzt wie die .env-Variante auf chmod 640 gesetzt (enthält DB-Zugangsdaten, vorher ungeschützt). Kritischer Sicherheitsfix: install.php verweigert jetzt von vornherein die Ausführung, falls bereits eine config.php existiert (renderAlreadyInstalledPage(), kein Bypass-Parameter) - vorher hätte ein liegen gebliebenes install.php (z.B. weil das automatische Selbst-Löschen in selfDestructAndRedirect() aus irgendeinem Grund fehlschlug) auf einer LIVE-Installation jedem, der die URL findet, erlaubt, über das Formular einen neuen Administrator-Zugang mit selbstgewähltem Passwort zu setzen.
- Changelog: 2.7.1 - Zwei tote, folgenlose Felder aus der .env-Vorlage entfernt (DEFAULT_LANGUAGE="de", DEFAULT_TEMPLATE="default") - beide wurden von config_loader.php nie ausgelesen (weder Env::get('DEFAULT_LANGUAGE', ...) noch Env::get('DEFAULT_TEMPLATE', ...) existiert im Code); die tatsächliche Sprach- bzw. Template-Auswahl läuft ausschließlich über die gleichnamigen, aber eigenständigen PHP-Konstanten in lang/i18n.php bzw. src/Template/TemplateEngine.php/frontend/template_engine.php. Reine Aufräumarbeit ohne Verhaltensänderung - die Felder täuschten eine Konfigurationsmöglichkeit vor, die es in Wirklichkeit nie gab.
...
- Changelog: 1.46.0 - Neuer Sprachschluessel ld_btn_overview ("Spieltage-Uebersicht") fuer den neuen Navigations-Link zur Spieltage-Uebersicht (siehe admin/view_liga_nav.php 1.0.0).
- Changelog: 1.45.0 - Neuer Schlüssel sp_placeholder_gt_grund für das neue Freitextfeld zur Grüne-Tisch-Entscheidung.
- Changelog: 1.44.0 - Neue Schlüssel sp_gt_beide (Dropdown-Option) und ls_gt_label_beide_verlieren (Einstellungsfeld) für die neue Grüne-Tisch-Entscheidung "Beide Mannschaften verlieren".
- Changelog: 1.43.0 - Neue Schlüssel ls_tickerart_ergebnisse_favorit/ls_label_tickernotizen/ls_hint_tickernotizen/ls_label_tickerbreite/ls_hint_tickerbreite/ls_label_tickergeschwindigkeit/ls_hint_tickergeschwindigkeit für die erweiterten Ticker-Einstellungen.
...
- Changelog: 1.45.0 - New language key ld_btn_overview ("Matchday overview") for the new navigation link to the matchday overview (see admin/view_liga_nav.php 1.0.0).
- Changelog: 1.44.0 - New key sp_placeholder_gt_grund for the new free-text field for the GT decision.
- Changelog: 1.43.0 - New keys sp_gt_beide and ls_gt_label_beide_verlieren for the new "both teams lose" GT decision.
- Changelog: 1.42.0 - New keys ls_tickerart_ergebnisse_favorit/ls_label_tickernotizen/ls_hint_tickernotizen/ls_label_tickerbreite/ls_hint_tickerbreite/ls_label_tickergeschwindigkeit/ls_hint_tickergeschwindigkeit for the extended ticker settings.
...
- Changelog: 1.10.0 - KRITISCHER Bugfix (gemeldet: "Core-Version zu niedrig. Benötigt: 1.9.2, vorhanden: 1.9.2-beta" bei einem Addon, das gegen genau diese Core-Version bereits erfolgreich getestet worden war): neue Methode versionSatisfiesMin() ersetzt den direkten version_compare(LMONEXT_VERSION, $minCore, '<')-Aufruf an beiden Stellen, an denen die Kompatibilität geprüft wird (hier bei der Installation, sowie handler_addons.php 1.5.0 beim Aktivieren). PHPs version_compare() stuft einen Pre-Release-Suffix (-beta/-alpha/-rc/-dev) als "kleiner" als dieselbe Zahl ohne Suffix ein - technisch korrekt nach SemVer-Konvention, hier aber falsch angewendet: der Suffix beschreibt die tatsächlich laufende Core-Version selbst ("1.9.2-beta" ist die Version, die läuft), nicht eine andere, tatsächlich ältere Version. Betraf JEDES Addon mit einer min_core_version ohne "-beta"-Suffix (praktisch alle), solange der Core-Versionsstring selbst ein "-beta"-Suffix trägt (seit composer.json 1.9.2-beta durchgängig der Fall) - keine Installation/Aktivierung eines aktuellen Addons wäre mehr möglich gewesen. Fix: versionSatisfiesMin() entfernt einen bekannten Pre-Release-Suffix vor dem Vergleich von BEIDEN Seiten (laufende Version UND Anforderung), sodass "1.9.5-beta" korrekt als >= "1.9.2" gilt, eine echte Unterschreitung (z.B. "1.9.0-beta" gegen "1.9.2") aber weiterhin korrekt erkannt wird. Mit vier Testfällen verifiziert (siehe CHANGELOG-Historie).
- Changelog: 1.9.0 - KRITISCHER Bugfix (gefunden bei der Integrationsprüfung eines Drittanbieter-Addons mit eigenem Hook-Handler, der trotz korrekter Manifest-Deklaration nie ausgelöst wurde): registerAddonHooks() prüft is_callable($handler), bevor der Hook registriert wird - das schlägt IMMER fehl, wenn die Handler-Funktion erst in einer admin_handlers/frontend_handlers-Datei DESSELBEN Addons definiert wird, da registerAddonHooks() (seit Version 1.8.0) VOR dem Laden dieser Handler-Dateien lief. Betraf potenziell jedes Addon mit einem im Manifest deklarierten Hook, dessen Handler-Funktion in einer eigenen Handler-Datei liegt (nicht nur das gerade geprüfte Addon) - nicht neu durch Version 1.8.0 verursacht, sondern ein vorbestehender Bug, der durch die dortige Umstrukturierung sichtbar/nachvollziehbar wurde. Fix: bootAdmin() und bootFrontend() laufen jetzt in DREI statt zwei Phasen - 1) Sprachen/Templates für alle Addons, 2) ERST DANACH alle Handler-Dateien laden (definiert alle Funktionen), 3) ERST JETZT alle Hooks registrieren. is_callable() findet die Handler-Funktion damit garantiert, unabhängig davon in welcher Datei/welchem Addon sie liegt.
- Changelog: 1.8.0 - KRITISCHER Bugfix (gemeldet: Sprachschlüssel wie "liga_h2h_modal_title" erschienen roh statt übersetzt im Teamvergleich-Modal, obwohl das teamvergleich-Addon seine eigene Sprachdatei korrekt hatte und mit korrektem Namen lud): bootAdmin() und bootFrontend() liefen bisher als EINE EINZIGE Schleife pro Addon, die sofort nacheinander loadLanguages() UND die Handler-Datei desselben Addons ausführte. loadTranslations() (lang/i18n.php) hat aber einen prozessweiten Cache pro Domain+Sprache, der beim ERSTEN t()/tf()-Aufruf befüllt und danach für den gesamten Request wiederverwendet wird - rief die Handler-Datei EINES FRÜHEREN Addons in der Iterations-Reihenfolge selbst t()/tf() auf (z.B. für eigene Initialisierung), wurde der Cache dadurch OHNE die Übersetzungen aller noch nicht verarbeiteten, SPÄTEREN Addons befüllt. Deren eigene t()/tf()-Aufrufe lieferten danach für den Rest des Requests weiterhin den rohen Schlüssel zurück, obwohl sie ihre Sprache korrekt (und mit richtigem Namen) registriert hatten - der bereits befüllte Cache wird ja wiederverwendet, nicht neu berechnet. Fix: beide Methoden laufen jetzt als ZWEI GETRENNTE Durchläufe - zuerst für ALLE Addons Sprachen/Templates/Hooks registrieren, ERST DANACH irgendeine Handler-Datei ausführen. So ist garantiert, dass jede Addon-Übersetzung im Cache steht, bevor überhaupt der erste t()/tf()-Aufruf aus einer Handler-Datei stattfinden kann - unabhängig von der Entdeckungs-/Iterations-Reihenfolge der Addons. Reiner Core-Fix, kein einzelnes Addon musste dafür geändert werden.
- Changelog: 1.7.0 - KRITISCHER Bugfix (gemeldet: Addon-Übersetzung für eine dritte Sprache "nl" war im Übersetzungs-Editor vollständig ausgefüllt, erschien aber trotzdem nicht im laufenden System - nur der Core-Admin-Bereich selbst schaltete korrekt auf Niederländisch um): loadLanguages() durchlief bisher eine hartkodierte Liste `['de', 'en']` - jede Addon-Sprachdatei für eine andere Sprache wurde dadurch komplett ignoriert, unabhängig davon, ob sie physisch existierte. Betraf JEDES Addon, nicht nur den Übersetzungs-Editor selbst. Jetzt an AVAILABLE_LANGUAGES gekoppelt (siehe lang/i18n.php 1.3.0) - die zentrale, bewusst gepflegte Liste der tatsächlich unterstützten Sprachen, statt eines hartkodierten Duos oder eines unkontrollierten Verzeichnis-Scans (der auch versehentlich angelegte, unvollständige Sprachdateien sofort aktiv schalten würde). Verifiziert: lang/i18n.php wird sowohl im Admin- als auch im Frontend-Bootstrap zuverlässig VOR bootAdmin()/bootFrontend() geladen, AVAILABLE_LANGUAGES ist zum Zeitpunkt des Aufrufs garantiert bereits definiert.
...
- Changelog: 1.2.0 - Neues Feature (auf Wunsch, im Rahmen der Integrationsprüfung des ewige-tabelle-Addons): neue Methode resolveTeamLinkGroups() berücksichtigt bereits im Admin-Bereich hinterlegte Team-Verknüpfungen (Umbenennung/Fusion/Abspaltung, siehe team_links-Tabelle, admin/bootstrap.php addTeamLink()). eternalStandings() und seasonMatrix() fassen verknüpfte Teams jetzt automatisch unter ihrer kanonischen (aktuellen) Team-ID zusammen statt sie getrennt zu zählen; eternalStandings() liefert zusätzlich das Feld 'former_names' (Namen, unter denen dieselbe verknüpfte Mannschaft in den ausgewählten Ligen sonst noch auftrat) - die Darstellung selbst obliegt bewusst dem aufrufenden Addon, dieser Core-Service besitzt keine eigenen Sprachschlüssel. Auflösung nach demselben Prinzip wie HeadToHead::resolveLinkedTeamIds()/resolveCanonicalTeamId() im teamvergleich-Addon (transitive BFS-Gruppierung, dann "wird abgelöst durch"-Kette über newer_team_id auflösen, Fallback bei uneindeutiger Richtung: höchste Team-ID der Gruppe), hier unabhängig implementiert - team_links ist eine reine Core-Tabelle, es besteht bewusst keine Abhängigkeit zwischen den beiden Standalone-Addons.
...
- Changelog: 1.26.0 - renderGtFootnotes() hängt jetzt einen vorhandenen gt_grund-Zusatztext an den bestehenden Fußnotentext an (auf Wunsch) - als eigener, optisch abgesetzter Satz ("Grund: ..."), gemeinsam für beide Entscheidungs-Zweige (Sieger-Fall und "beide verlieren"-Fall) umgesetzt, um den Code nicht zu duplizieren.
...
- Changelog: 1.24.0 - renderTickerBlock() und renderTickerErgebnisText() komplett entfernt (auf Wunsch als eigenständiges Addon "ticker" ausgegliedert, siehe frontend/data_liga.php 3.5.0 für den neuen Hook-Wrapper) - leben jetzt unverändert in addon/ticker/TickerRenderer.php.
...
- Changelog: 1.6.0 - Auf Wunsch: getSpieltagPartien() sortiert Spiele eines Spieltags jetzt primär chronologisch nach Anstoßzeit (p.zeit, älteste zuerst, Spiele ohne Zeit rutschen ans Ende statt an den Anfang) statt nach der bisherigen rein technischen spiel_nr-Reihenfolge (Anlege-/Import-Reihenfolge) - diese bleibt als Tie-Breaker bei gleicher/fehlender Zeit erhalten, für eine stabile Reihenfolge.
- Changelog: 1.5.0 - getSpieltagPartien() lädt jetzt zusätzlich die Spalte gt_entscheidung mit (gleiches defensives "Spalte existiert?"-Muster wie bei status/nicht_gewertet) - Grundlage für die neue Grüne-Tisch-Entscheidung, siehe admin/bootstrap.php.
- Changelog: 1.4.0 - getSpieltagPartien() lädt jetzt zusätzlich die Spalte nicht_gewertet mit (gleiches defensives "Spalte existiert?"-Muster wie bei status/extra_data, damit eine frische Installation vor der Migration nicht scheitert) - Grundlage für die neue Annullieren-Funktion, siehe admin/bootstrap.php.
...
- Changelog: 1.1.0 - statusSuffix() hängt jetzt zusätzlich "nicht gewertet" an, wenn ein Spiel rückwirkend annulliert wurde (nicht_gewertet=1, unabhängig vom bestehenden status-Wert für i.E./n.V.) - das bestehende Anzeigeformat für den bisherigen Einzelfall (nur i.E./n.V.) bleibt dabei bewusst unverändert.
...
- Changelog: 1.1.0 - KRITISCHER Bugfix (gemeldet: nach einem Re-Import über das url-import-Addon zeigte favTeam/selTeam plötzlich auf ein völlig anderes Team, z.B. FC Bayern München statt des zuvor eingestellten FC Schalke 04): favTeam/selTeam speicherten bisher NICHT die stabile teams_global.id, sondern nur die Position dieses Teams innerhalb der nach Name sortierten Teamliste der Liga - änderte sich die Menge/Schreibweise der Teams (z.B. durch einen Re-Import, der ein Team mit leicht anderem Namen neu anlegt), verschob sich die alphabetische Position aller danach folgenden Teams, dieselbe gespeicherte Zahl zeigte danach auf ein anderes Team. resolveTeamNumberToId() umgebaut von "Positions-Auflösung" auf reine Gültigkeits-Prüfung (ist diese ID Teil der Liga?) - admin/view_liga_settings.php speichert jetzt direkt die echte team_id. Neue Funktion migrateFavSelTeamToStableId(): konvertiert bereits gespeicherte, alte Positions-Werte einmalig pro Liga in die zum Migrationszeitpunkt damit gemeinte ID (mit der historischen "sortiert nach Name"-Logik), eigenes Flag pro Liga (FavSelTeamMigrated) statt eines globalen Schalters. Mit einer simulierten Datenbank verifiziert: Migration korrekt, idempotent bei erneutem Aufruf, und der gemeldete Bug tritt nach der Migration auch bei einem weiteren Re-Import mit neu eingefügtem Team nicht mehr auf.
...
- Changelog: 1.17.16 - .liga-ticker-CSS komplett entfernt (auf Wunsch als eigenständiges Addon "ticker" ausgegliedert) - lebt jetzt als Inline-<style>-Block im Hook-Ergebnis (addon/ticker/TickerRenderer.php), da das Addon keinen festen Platz mehr im Core-Template-CSS hat.
...
- Changelog: 1.5.0 - $istGtGewertet erkennt jetzt auch gt_entscheidung=3 ("beide Mannschaften verlieren", siehe StandingsTrait.php 1.12.0) - zeigt "0:0" mit dem bekannten Fußnoten-Sternchen statt des realen Ergebnisses.
- Changelog: 1.4.0 - Default-Fallback für _gt_tore_nichtantritt von 3 auf 2 geändert, siehe StandingsTrait::gtCreditedScore() 1.11.0.
- Changelog: 1.3.0 - formatResult() übergibt die pro Liga einstellbaren Grüne-Tisch-Standardwerte (siehe liga.php 3.17.0, _gt_tore_gespielt/_gt_tore_nichtantritt) an gtCreditedScore() statt der Funktions-Defaults - damit wirkt die Liga-Einstellung auch in der Ergebnisanzeige, nicht nur in der Tabellenberechnung.
...
- Changelog: ENTFERNT - diese Datei wurde gelöscht, der Ticker ist auf Wunsch als eigenständiges Addon "ticker" ausgegliedert (siehe CHANGELOG-Einträge zu src/Liga/RenderViewsTrait.php 1.24.0 und frontend/data_liga.php 3.5.0). Die Markup-Struktur lebt jetzt (leicht angepasst, als PHP-String statt Platzhalter-Template) in addon/ticker/TickerRenderer.php.
...
- Changelog: ENTFERNT - gleicher Hintergrund wie template/default/partials/ticker_block.tpl.php.
