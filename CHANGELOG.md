...
- Changelog: 1.25.0 - Neues Feature (nach DFB-Rechts- und Verfahrensordnung): ensureSpielstatusColumns() erweitert um die Spalte `gt_entscheidung` auf liga_partien (0=keine, 1=Heimteam siegt, 2=Gastteam siegt am grünen Tisch) - siehe StandingsTrait::gtCreditedScore() für die vollständige Wertungsregel.
- Changelog: 1.24.0 - Neues Feature: ensureSpielstatusColumns() erweitert um die additive Spalte `nicht_gewertet` auf liga_partien - erlaubt es, ein Spiel rückwirkend zu annullieren (zählt für keines der beiden beteiligten Teams in der Tabelle), z.B. bei Lizenzentzug/Spielbetrieb-Einstellung eines Vereins mitten in der Saison. Bewusst eine EIGENE, von "status" (i.E./n.V.) unabhängige Spalte statt eines dritten status-Werts, damit ein annulliertes Elfmeterschießen-Spiel diese Information nicht verliert, falls die Annullierung später wieder aufgehoben wird.
...
- Changelog: 1.10.0 - Neuer Eingabebereich im Strafen-Tab: zwei Zahlenfelder für die pro Liga einstellbare Grüne-Tisch-Standardwertung ("Spiel fand statt"/"Nichtantritt", siehe StandingsTrait::gtCreditedScore() 1.10.0 und admin/handler_settings.php 1.8.0), vorbelegt mit den bisherigen DFB-Standardwerten 2 bzw. 3 über den bereits vorhandenen $o()-Optionen-Helfer.
- Changelog: 1.9.0 - Neues Feature im Strafen-Tab: neue Spalte "Alle Spiele annullieren" mit einer Checkbox pro Team - setzt beim Speichern ALLE Spiele dieses Teams in dieser Liga auf "nicht gewertet" (siehe admin/handler_settings.php). Bereits annullierte Teams werden farblich hervorgehoben angezeigt.
...
- Changelog: 1.3.3 - zurückgenommen: Schlüsselplan-Option wieder wie ursprünglich immer sichtbar (nur ausgegraut mit "nicht verfügbar"-Hinweis) statt komplett zu verschwinden, wenn kein Muster zur Teamzahl passt
...
- Changelog: 1.11.1 - Core-Versionssprung für die Stilkorrektur PHP-Kurzsyntax → geschweifte Klammern (siehe addon/addon-manager/view_addons.php 2.5.0 und admin/view_liga_settings.php 1.18.0). Reine Code-Stilfrage, keine Verhaltensänderung.

- Changelog: 1.11.0 - Core-Versionssprung für den neuen "Entdecken"-Tab im Addon-Manager (siehe addon/addon-manager/ - addon.json 1.8.0/view_addons.php 2.4.0/catalog.php 1.0.0): offizieller Addon-Katalog mit Direktinstallation per Klick.

- Changelog: 1.10.2 - Core-Versionssprung für die drei install.php-Härtungen (siehe install.php 2.9.0): CSRF-Schutz fürs Installationsformular, explizite MySQL/MariaDB-Mindestversionsprüfung, Bestätigungs-Checkbox bei bereits vorhandener Installation unter demselben Host/DB-Name/Präfix.

- Changelog: 1.10.1 - Core-Versionssprung für die install.php-Überarbeitung (siehe install.php 2.8.0): vollständigeres DB-Schema von Anfang an (team_links, addon_registry, addon_settings + mehrere fehlende Spalten), Rechte-Vorabprüfung, abschließende Konsistenzprüfung, entfernter bin/composer.phar-Rückfall (bin/-Ordner komplett aus dem Core gelöscht) sowie mehrere Sicherheits-Härtungen (config.php-Dateiberechtigung, Schutz vor Neuinstallation über eine bestehende config.php).
- Changelog: 1.10.0 - Hochstufung aus der Beta-Phase: kein "-beta"-Suffix mehr. Rein administrative Versionsänderung (nach ausgiebigem beschwerdefreiem Betrieb) - keine Code-Änderung, da die Version überall (Footer, Admin-Sidebar, Backup-Dateien, Addon-Kompatibilitätsprüfung) einheitlich per getAppVersion()/renderCopyrightNotice() direkt aus composer.json gelesen wird. AddonManager::versionSatisfiesMin() (1.10.0, siehe dortiger Changelog) entfernt Pre-Release-Suffixe ohnehin schon vor dem Vergleich, ein Addon mit min_core_version "1.10.0-beta" bleibt also weiterhin kompatibel.
- Changelog: 1.10.0-beta - Core-Versionssprung für das neue "Liga öffnen springt direkt zum aktuellen Spieltag"-Feature (siehe admin/bootstrap.php 1.28.0, admin/data_loader.php 1.15.0, neues Partial admin/view_liga_nav.php 1.0.0, admin/view_spieltag.php 1.9.0, admin/view_liga_detail.php umbenannt zu admin/view_liga_spieltage.php 1.0.0).
...
- Changelog: 1.55.0 - umformuliert: liga_gt_footnote_heading von "(*) Grüne-Tisch-Entscheidungen" auf "Grüne-Tisch-Entscheidung" (kein "(*)"-Präfix, Singular), liga_gt_footnote_line von "endete mit {real}" auf "endete {real}" (ohne "mit").
- Changelog: 1.54.0 - liga_gt_footnote_line_no_real umformuliert: "{nicht_angetreten} ist nicht angetreten. Das Spiel wird mit {gewertet} für {sieger} gewertet." statt der bisherigen, weniger konkreten Formulierung - benennt jetzt direkt das nicht angetretene Team.
...
- Changelog: 1.2.0 - zurückgebaut: Logo steht wieder über dem Teamnamen (gestapelt) statt daneben – bei langen Namen wie "Bayer 04 Leverkusen" führte die horizontale Anordnung zu abgeschnittenem Text ("Bayer 04 Leve..."). Gleiche Rückänderung im "Vorheriges Spiel"-Block
- Changelog: 1.1.0 - Team-Anzeige umgebaut: Logo steht jetzt neben dem Namen statt darüber ("TEAMNAME LOGO -:- LOGO TEAMNAME", Logos "schauen" zum Ergebnis in der Mitte), analog zur Ergebnisse-Ansicht der normalen Besucherseite. CSS-Selektor für die Logo-Größe generisch auf "img im Team-Bereich" gesetzt statt auf die globale .team-logo-inline-Klasse (existiert auf dieser eigenständigen Seite nicht). Gleiche Anpassung im "Vorheriges Spiel"-Block
...
- Changelog: 2.2.0 - Neuer Platzhalter <!--TickerBlock--> zwischen Titel und Tab-Leiste eingefügt (siehe src/Liga/RenderViewsTrait.php 1.22.0).
...
- Changelog: 1.6.0 - Trend-Spalte (.st-trend) von der letzten Position an die zweite Stelle verschoben, direkt zwischen Platz und Teamname.
...
- Changelog: 1.5.0 - Trend-Spaltenkopf (ColTrend) von der letzten Position an die zweite Stelle verschoben, direkt zwischen Platz und Teamname - analog zur selben Änderung in standings_row.tpl.php.
...
- Changelog: 1.2.0 - Neuer Platzhalter <!--TickerBlock--> zwischen Titel und Tab-Leiste eingefügt (siehe src/Liga/RenderViewsTrait.php 1.22.0).
...
- Changelog: 1.3.0 - Trend-Spalte (.st-trend) von der letzten Position an die zweite Stelle verschoben, direkt zwischen Platz und Teamname.
...
- Changelog: 1.4.0 - Trend-Spaltenkopf (ColTrend) von der letzten Position an die zweite Stelle verschoben, direkt zwischen Platz und Teamname - analog zur selben Änderung in standings_row.tpl.php.
...
- Changelog: 1.0.0 - Neues Partial: Fußnoten-Box unterhalb der Ergebnistabelle eines Spieltags, erklärt jede Grüne-Tisch-Entscheidung im Klartext. Platzhalter Heading + Lines, siehe RenderViewsTrait::renderGtFootnotes() für den PHP-seitigen Aufbau.
