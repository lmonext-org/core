...
- Changelog: 1.9.0 - Speicherlogik für neue Einstellung "show_teamvergleich" ergänzt (Beitrag: Nutzerwunsch, Teamvergleich als eigenständiges Addon "teamvergleich" ausgegliedert, siehe frontend/data_liga.php 3.1.0). Analog zu show_pdf_buttons per isset($_POST[...]) geschützt - kein Reset-Risiko.
...
- Changelog: 1.8.0 - Neues Feld "show_teamvergleich" (Tab Anzeigen/Darstellung, Wortlaut/Hinweistext analog zu show_pdf_buttons) - steuert erstmals, ob Besucher das Teamvergleich-Icon bei Begegnungen sehen (gab es bisher nicht als Einstellung, das Icon erschien immer). Nur sichtbar, wenn das neue teamvergleich-Addon aktiviert ist (Beitrag: Nutzerwunsch, Auslagerung als eigenständiges Addon, siehe frontend/data_liga.php 3.1.0).
- Changelog: 1.7.0 - "PDF-Buttons anzeigen"-Feld (show_pdf_buttons, Tab Anzeigen/Darstellung) erscheint jetzt nur noch, wenn das neue pdf-export-Addon aktiviert ist (Beitrag: Nutzerwunsch, Auslagerung als eigenständiges Addon, siehe liga.php 3.13.0). Speicherung in admin/handler_user.php war bereits sicher (nutzt isset($_POST[...]) - kein Reset-Risiko wie beim früheren "stats"-Checkbox-Bug), daher dort keine Änderung nötig.
...
- Changelog: 2.23.0 - Die komplette Duplikat-Implementierung der H2H-Funktionen (resolveLinkedTeamIds, resolveCanonicalTeamId, getHeadToHeadMatches, buildHeadToHeadPayload, renderH2hIcon, renderH2hModalAssets - ca. 440 Zeilen, eine reine Code-Duplikation der gleichnamigen Funktionen in frontend/data_liga.php) komplett ENTFERNT und durch identische Hook-Wrapper ersetzt (siehe frontend/data_liga.php 3.1.0 für die vollständige, kommentierte Version) - Teil der Auslagerung des Teamvergleichs als eigenständiges Addon "teamvergleich" (Beitrag: Nutzerwunsch). Beseitigt nebenbei die seit Langem bestehende Code-Duplikation zwischen dieser Datei und data_liga.php für diesen Funktionsbereich.
- Changelog: 2.22.0 - renderH2hModalAssets(): der PDF-Button IM Teamvergleich-Popup prüft jetzt zusätzlich den Status des neuen pdf-export-Addons (Beitrag: Nutzerwunsch, Auslagerung als eigenständiges Addon, siehe liga.php 3.13.0) - ohne diese Ergänzung wäre der Button trotz deaktiviertem Addon weiterhin sichtbar gewesen, nur der Klick darauf wäre ins Leere gelaufen (liga.php blockte den eigentlichen h2h_pdf-Aufruf bereits). Identische Ergänzung in der parallelen Implementierung src/Liga/HeadToHeadTrait.php::renderH2hModalAssets() nötig (siehe dortiger Changelog-Eintrag) - beide enthalten dieselbe H2H-Modal-Logik dupliziert.
...
- Changelog: AUSGEGLIEDERT (Version 1.9.1 war der letzte Core-Stand) - diese Datei existiert im Core nicht mehr, siehe liga.php 3.13.0: PDF-Export wurde komplett als eigenständiges Addon "pdf-export" ausgegliedert (Beitrag: Nutzerwunsch). Neuer Speicherort: addon/pdf-export/pdf_export.php, eigenes CHANGELOG.md dort.
...
- Changelog: 1.1.0 - "use HeadToHeadTrait;" entfernt (Beitrag: Nutzerwunsch, Auslagerung des Teamvergleichs als eigenständiges Addon "teamvergleich", siehe frontend/data_liga.php 3.1.0) - die Logik lebt jetzt unter addon/teamvergleich/HeadToHead.php als eigenständige Klasse, nicht mehr als in diese Fassade eingemischte Trait.
