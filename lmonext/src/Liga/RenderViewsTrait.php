...
trait RenderViewsTrait
{
    ...
    public static function renderGtFootnotes(array $partien) : string
    {
        ...
        foreach ($partien as $p) {
            ...
            if ($gtEntscheidung === 3) {
                // Beide Mannschaften verlieren - eigener
                ...
                if ($hTore !== null && $gTore !== null) {
                    ...
                    // Entscheidung: Heimteam siegt -> Gastteam ist
            ...
            // Freier Zusatztext zur Entscheidung (gt_grund) -
    ...
    public static function renderStandingsView(int $ligaId, array $allSpieltage, ?int $uptoSpieltag = null, string $tableMode = 'gesamt') : string
    {
        ...
        // Ungefiltert (inkl. künftiger Spieltage) für den 6. "nächste
        // Begegnung"-Pfeil (siehe computeNextMatchDot() in
        // StandingsTrait.php) - $partien wird direkt darunter auf "<= $nr"
        // gefiltert und ist ab dann für diesen Zweck nicht mehr brauchbar.
        $allPartienUnfiltered = $partien;
        ...
        foreach ($rows as $i => $r) {
            ...
            $rowsHtml .= renderPartial('standings_row', [
                ...
                'Form'     => ($formByTeam[$tid]['dots'] ?? '') . ($nextDotByTeam[$tid] ?? ''),
                'Trend'    => $trendHtml,
            ]);
        }
    ...
    /**
     * Ticker (als eigenständiges Addon "ticker" ausgegliedert,
     * siehe frontend/data_liga.php renderTickerBlock() für den neuen, davor
     * bewusst schlanken Hook-Wrapper - Core kennt den Ticker jetzt nur noch
     * über den Hook "liga.ticker_block", kein Fallback ohne aktives Addon).
     * Frühere Implementierung dieser Klasse (renderTickerBlock()/
     * renderTickerErgebnisText()) lebt jetzt in addon/ticker/TickerRenderer.php.
     */
